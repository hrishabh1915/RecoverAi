...
function isFailureReason(value) {
  return value === "NETWORK_ERROR" || value === "BANK_DECLINED_GENERIC" || value === "INSUFFICIENT_FUNDS" || value === "EXPIRED_CARD" || value === "AUTH_FAILURE" || value === "FRAUD_SUSPECTED" || value === "UNKNOWN";
}

// backend/src/types/prismaMapping.ts
var paymentStatusToDomainMap = {
  PENDING: "pending",
  SUCCEEDED: "succeeded",
  FAILED: "failed",
  RECOVERED: "recovered"
};
function paymentStatusToDomain(value) {
  return paymentStatusToDomainMap[value];
}
function paymentStatusToPrisma(value) {
  switch (value) {
    case "pending":
      return "PENDING";
    case "succeeded":
...
function failureReasonToDomain(value) {
  return value;
}
function failureReasonToPrisma(value) {
  return value;
}
function paymentMethodToDomain(value) {
...
function toPaymentRecord(row) {
  return {
    ...
    status: paymentStatusToDomain(row.status),
    failureReason: row.failureReason ? failureReasonToDomain(row.failureReason) : null,
...
async function createPayment(data) {
  const row = await prisma.payment.create({
    data: {
      ...
      transactionCreatedAt: typeof data.transactionCreatedAt === "string" ? new Date(data.transactionCreatedAt) : data.transactionCreatedAt,
      recoveredAt: data.recoveredAt ? typeof data.recoveredAt === "string" ? new Date(data.recoveredAt) : data.recoveredAt : null
    }
  });
...
async function getPaymentByGatewayPaymentId(gatewayPaymentId) {
...
async function getPaymentForExecution(transactionId) {
  const row = await prisma.payment.findUnique({
...
async function getCustomerByExternalId(merchantId, externalCustomerId) {
  const row = await prisma.customer.findUnique({
    where: {
...
async function getCustomerState(customerId) {
  ...
  const status = isBlocked ? "blocked" : isFlagged ? "flagged" : "active";
...
// backend/src/repositories/processedEventRepository.ts
function toProcessedEventRecord(row) {
  let parsedPayload = row.rawPayload;
  try {
...
async function isEventProcessed(merchantId, gatewayEventId) {
...
async function insertProcessedEvent(merchantId, gatewayEventId, paymentId, eventReceivedAt, rawPayload) {
  ...
  const row = await prisma.processedEvent.create({
    data: {
      ...
      gatewayEventId,
      paymentId,
      eventReceivedAt: typeof eventReceivedAt === "string" ? new Date(eventReceivedAt) : eventReceivedAt,
...
// backend/src/repositories/recoveryAttemptRepository.ts
...
async function getActiveAttemptForPayment(paymentId) {
...
async function getAttemptsForPayment(paymentId) {
...
async function createRecoveryAttempt(paymentId, retryCount, startedAt) {
...
async function updateAttemptStatus(id, status, completedAt) {
  const row = await prisma.recoveryAttempt.update({
    where: { id },
    data: {
      status,
      completedAt: completedAt ? typeof completedAt === "string" ? new Date(completedAt) : completedAt : status === "completed" || status === "failed" || status === "cancelled" ? /* @__PURE__ */ new Date() : null
    }
  });
  return toRecoveryAttemptRecord(row);
}
...
var AIDiagnosisEnum = external_exports.enum([
  "NETWORK_ERROR",
...
var AIResponseSchema = external_exports.object({
  ...
  recommendedAction: external_exports.enum(["SMART_RETRY", "CUSTOMER_RECOVERY", "HUMAN_REVIEW", "NO_ACTION"]),
  reasoningFactors: external_exports.array(external_exports.string()).max(5)
}).strict();
...
// backend/src/repositories/recoveryDecisionRepository.ts
...
async function insertRecoveryDecision(insert) {
  ...
  const row = await prisma.recoveryDecision.create({
    data: {
      ...
      evaluationTime: typeof insert.evaluationTime === "string" ? new Date(insert.evaluationTime) : insert.evaluationTime,
...
async function getDecisionsForPayment(paymentId) {
...
async function insertAuditLog(insert) {
  const detailsString = typeof insert.details === "string" ? insert.details : JSON.stringify(insert.details ?? {});
...
// backend/src/repositories/executionContextBuilder.ts
async function buildExecutionContext(transactionId) {
...
// backend/src/config/policyConfig.ts
var POLICY_CONFIG = Object.freeze({
  maxRetryLimit: 3,
  cooldownPeriodMinutes: 30,
  highValueThresholdMinor: 5e6,
...
// backend/src/services/scoreCalculator.ts
function calculateFailureReasonScore(reason) {
  switch (reason) {
    case "NETWORK_ERROR":
      return 95;
...
function calculateRetryScore(retryCount) {
  if (retryCount <= 0) return 100;
  if (retryCount === 1) return 70;
  if (retryCount === 2) return 35;
  return 0;
}
function calculateRecencyScore(delayMinutes) {
...
function calculateHistoryScore(previousSuccessful, previousFailed, isRisk) {
  if (isRisk) return 0;
...
function calculateConfidenceScore(confidence, isValid2) {
...
function calculateRecoveryScore(input) {
  ...
  const delayMinutes = Math.max(0, (eventReceived - txCreated) / (1e3 * 60));
  ...
  const weightedSum = failureReasonScore * weights.failureReason + retryScore * weights.retry + recencyScore * weights.recency + historyScore * weights.history + confidenceScore * weights.confidence;
...
function evaluatePolicy(input, score, evaluationTime = /* @__PURE__ */ new Date()) {
  ...
  if (input.lastRetryAt) {
    ...
    const elapsedMinutes = (evalMs - lastRetryTime) / (1e3 * 60);
    if (elapsedMinutes < POLICY_CONFIG.cooldownPeriodMinutes) {
      const eligibleTime = new Date(
        lastRetryTime + POLICY_CONFIG.cooldownPeriodMinutes * 60 * 1e3
      );
      return {
        decision: "DENY",
        action: "NO_ACTION",
  ...
  const eventDeliveryDelayMinutes = Math.max(0, (eventReceivedMs - txCreatedMs) / (1e3 * 60));
...
// backend/src/services/aiDiagnosisService.ts
function buildDiagnosticPrompt(ctx) {
...
function runSemanticDiagnosticEngine(ctx) {
  let diagnosis = ctx.failureReason;
  let confidence = 0.85;
  let recommendedAction = "SMART_RETRY";
...
function processRawLLMOutput(rawText, provider = "gemini_api") {
  try {
...
function createSafetyFallback(reason, rawOutput) {
  return {
    aiResponse: {
      diagnosis: "UNKNOWN",
      confidence: 0,
...
// backend/src/services/recoveryPipeline.ts
async function processPaymentFailureEvent(event) {
  const eventReceivedAt = event.eventReceivedAt ?? (/* @__PURE__ */ new Date()).toISOString();
  ...
  if (event.aiOverride) {
    ...
    const rawPayloadObj = typeof event.rawPayload === "object" && event.rawPayload !== null ? event.rawPayload : {};
    const errDesc = typeof rawPayloadObj["error_description"] === "string" ? rawPayloadObj["error_description"] : void 0;
    const errCode = typeof rawPayloadObj["error_code"] === "string" ? rawPayloadObj["error_code"] : void 0;
  ...
  const evaluationTime = /* @__PURE__ */ new Date();
  ...
  if (decision.decision === "ALLOW") {
    ...
    if (recoverySuccess) {
      await updateAttemptStatus(attempt.id, "completed", /* @__PURE__ */ new Date());
      ...
      await updateAttemptStatus(attempt.id, "failed", /* @__PURE__ */ new Date());
...
// backend/src/services/razorpayWebhookService.ts
...
function mapRazorpayFailureReason(errorCode, errorDescription, errorReason) {
  ...
  const combined = [errorCode, errorDescription, errorReason].filter(Boolean).join(" ").toLowerCase();
  if (combined.includes("insufficient") || combined.includes("funds") || combined.includes("balance")) {
    return "INSUFFICIENT_FUNDS";
  }
  if (combined.includes("auth") || combined.includes("otp") || combined.includes("2fa") || combined.includes("password")) {
    return "AUTH_FAILURE";
  }
  if (combined.includes("expired") || combined.includes("expiry")) {
    return "EXPIRED_CARD";
  }
  if (combined.includes("fraud") || combined.includes("risk") || combined.includes("blacklisted")) {
    return "FRAUD_SUSPECTED";
  }
  if (combined.includes("declined") || combined.includes("bank") || combined.includes("issuer")) {
    return "BANK_DECLINED_GENERIC";
  }
  if (combined.includes("timeout") || combined.includes("network") || combined.includes("connection") || combined.includes("gateway")) {
...
async function processRazorpayWebhookEvent(rawBody, payload, signatureHeader) {
  ...
  const createdAtUnix = paymentEntity.created_at ?? payload.created_at ?? Math.floor(Date.now() / 1e3);
  const gatewayEventId = payload.event_id ?? `event_rzp_${gatewayPaymentId}_${createdAtUnix}`;
  let merchantId = paymentEntity.notes?.merchant_id ?? payload.merchant_id;
  if (!merchantId) {
  ...
  const externalCustomerId = paymentEntity.customer_id ?? paymentEntity.email ?? paymentEntity.contact ?? "cust_rzp_default";
  ...
  const transactionCreatedAt = new Date(createdAtUnix * 1e3).toISOString();
  let payment = await getPaymentByGatewayPaymentId(gatewayPaymentId);
  if (!payment) {
  ...
  const pipelineResult = await processPaymentFailureEvent({
    ...
    gatewayEventId,
    paymentId: payment.id,
    eventReceivedAt: (/* @__PURE__ */ new Date()).toISOString(),
    rawPayload: payload
  });
  return {
    ...pipelineResult,
...
// backend/src/services/batchRecoveryService.ts
async function processBatchRecovery(options = {}) {
  ...
  for (const p of rawPayments) {
    ...
    const failureReason = p.failureReason ?? "UNKNOWN";
    if (isDryRun) {
      ...
      if (res.status === "RECOVERED") {
        ...
        statusAfter = "human_review_queued";
        if (res.decision) {
          stoppingReasons[res.decision.reasonCode] = (stoppingReasons[res.decision.reasonCode] ?? 0) + 1;
        }
      } else {
        stoppedOrDenied++;
        statusAfter = "recovery_denied";
  ...
  const recoveryRatePercent = totalAtRiskINR > 0 ? Math.round(totalRecoveredINR / totalAtRiskINR * 100) : 0;
  return {
    batchId,
    startedAt,
...
async function getPendingEscalations(merchantId) {
  const whereClause = {
...
async function approveEscalation(decisionId, operatorNotes = "Approved by human operator after review") {
  ...
  const attempt = await createRecoveryAttempt(decision.paymentId, 1, /* @__PURE__ */ new Date());
  await updateAttemptStatus(attempt.id, "completed", /* @__PURE__ */ new Date());
...
async function rejectEscalation(decisionId, operatorNotes = "Rejected by human operator: recovery not recommended") {
  const decision = await prisma.recoveryDecision.findUnique({
    where: { id: decisionId }
  });
  if (!decision) {
...
async function getPaymentAuditTimeline(paymentId) {
  ...
  for (const d of payment.recoveryDecisions) {
    ...
    timeline.push({
      ...
      timestamp: d.evaluationTime.toISOString(),
      badgeType: d.policyDecision === "ALLOW" ? "success" : d.policyDecision === "HUMAN_APPROVAL_REQUIRED" ? "warning" : "error",
      metadata: {
  ...
  for (const log of escalationLogs) {
    ...
    timeline.push({
      ...
      description: details["operatorNotes"] ?? "Decision recorded by human operator.",
  ...
  for (const att of payment.recoveryAttempts) {
    timeline.push({
      ...
      description: att.status === "completed" ? "Autonomous smart retry executed successfully. Funds captured." : att.status === "in_flight" ? "Recovery attempt locked in-flight." : "Recovery attempt failed on rail switch.",
  ...
  timeline.push({
    ...
    description: payment.status === "RECOVERED" ? `Successfully recovered \u20B9${amountINR.toLocaleString("en-IN")} on ${payment.paymentMethod.toUpperCase()} rail.` : "Payment stopped by safety policy or operator.",
...
app.get("/api/metrics", async (_req, res) => {
  try {
    ...
    const recoveryRate = totalFailed + totalRecovered > 0 ? Math.round(totalRecovered / (totalFailed + totalRecovered) * 100) : 0;
    res.json({
      metrics: {
        totalPayments,
        ...
        simulatedDisclosure: "Demo Environment: All \u20B9 recovered figures are simulated using synthetic Razorpay-shaped event payloads. AI recommends; deterministic policy disposes."
      },
      decisionsBreakdown,
      failureReasonCounts,
      paymentMethodCounts,
...
app.post("/api/ai/diagnose", async (req, res) => {
  try {
    ...
    const validatedFailureReason = typeof failureReason === "string" && isFailureReason(failureReason) ? failureReason : "UNKNOWN";
    const validatedPaymentMethod = typeof paymentMethod === "string" && isPaymentMethod(paymentMethod) ? paymentMethod : "card";
    const result = await diagnosePaymentFailure(
      ...
      rawMockOutput ? { rawMockOutput } : void 0
    );
    res.json(result);
  } catch (error) {
...
app.post("/api/recovery/batch", async (req, res) => {
  try {
    ...
    const batchResult = await processBatchRecovery({
      ...
      paymentIds: Array.isArray(paymentIds) ? paymentIds : void 0
    });
    res.json({
      success: true,
      batch: batchResult
...
app.get("/api/recovery/batch/summary", async (_req, res) => {
  try {
    ...
    for (const p of failedPayments) {
      ...
      if (p.failureReason) {
        failureReasonBreakdown[p.failureReason] = (failureReasonBreakdown[p.failureReason] ?? 0) + 1;
      }
    }
    res.json({
      summary: {
...
app.get("/api/payments", async (req, res) => {
  try {
    ...
    const payments = await getAllPayments({
      merchantId: typeof merchantId === "string" ? merchantId : void 0,
      status: validStatus,
...
app.get("/api/escalations", async (req, res) => {
  try {
    const merchantId = typeof req.query.merchantId === "string" ? req.query.merchantId : void 0;
...
function startServer(port = PORT) {
  return app.listen(port, () => {
    console.log(`
\u{1F680} RecoverAI Backend running at http://localhost:${port}`);
